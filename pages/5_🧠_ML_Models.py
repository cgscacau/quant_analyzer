...
def _is_num(x: Any) -> bool:
    """Retorna True se x for número finito (aceita str conversível)."""
    try:
...
def _fmt(x: Any, kind: str = "num", na: str = "N/A") -> str:
    """Formata com segurança valores numéricos."""
    if not _is_num(x):
        return na
    v = float(x)
    if kind == "num":
...
def _last_of_series(obj: Any) -> Optional[float]:
    """
    Tenta obter o último valor numérico de um pandas.Series, list, tuple,
    numpy array etc. Retorna None se não existir ou não for numérico.
    """
    try:
        # pandas.Series
        if hasattr(obj, "iloc") and hasattr(obj, "__len__"):
            if len(obj) == 0:
                return None
            val = obj.iloc[-1]
            return float(val) if _is_num(val) else None

        # numpy array / lista / tupla
...
def render_tactical_plan(
    sym: str,
    *,
    price: Any = None,            # preço atual
    p_series: Any = None,         # série de probabilidades (opcional)
    p_up_pred: Any = None,        # prob. atual isolada (opcional)
    rsi: Any = None,
    atr: Any = None,
    sw_hi: Any = None,            # swing high (20) opcional
    sw_lo: Any = None,            # swing low  (20) opcional
    fast_gt_slow: bool = False,   # médias móveis alinhadas (rápida > lenta)
    macd_pos: bool = False,       # MACD positivo?
    buy_threshold: Any = 0.55,    # limiar de compra para P(up)
    title_suffix: str = " — Resumo e plano tático",
    start_open: bool = True,
) -> None:
    """
    Renderiza um card tático seguro, tolerante a dados faltantes.
    Nunca levanta erro — se não houver dados, mostra 'N/A'.
    """

    # ---------- coleta segura dos valores ----------
    last_price = float(price) if _is_num(price) else math.nan

    # P(up): tenta série; se não houver, usa valor direto
    last_p_series = _last_of_series(p_series)
    if _is_num(last_p_series):
        p_up = float(last_p_series)
    elif _is_num(p_up_pred):
        p_up = float(p_up_pred)
    else:
        p_up = math.nan

    rsi_v   = float(rsi) if _is_num(rsi) else math.nan
    atr_v   = float(atr) if _is_num(atr) else math.nan
    sw_hi_v = float(sw_hi) if _is_num(sw_hi) else math.nan
    sw_lo_v = float(sw_lo) if _is_num(sw_lo) else math.nan
    th      = float(buy_threshold) if _is_num(buy_threshold) else 0.55

    # ---------- UI ----------
    with st.expander(f"🧭 {sym}{title_suffix}", expanded=start_open):
        # Métricas principais
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Preço",      _fmt(last_price))
        ...
        st.caption(
            f"Tendência: **{trend_txt}**  |  MACD: **{macd_txt}**  |  "
            f"Limiar de compra: **{_fmt(th)}**"
        )
        ...
        # Stop/Alvo: ATR se disponível; senão usa swing levels
        use_atr = _is_num(atr_v) and atr_v > 0
        stop = (last_price - 1.5 * atr_v) if use_atr and _is_num(last_price) else (
               sw_lo_v if _is_num(sw_lo_v) else math.nan)
        tgt  = (last_price + 3.0 * atr_v) if use_atr and _is_num(last_price) else (
               sw_hi_v if _is_num(sw_hi_v) else math.nan)

        rr = math.nan
        if _is_num(stop) and _is_num(tgt) and _is_num(last_price) and stop < last_price < tgt:
            risk = last_price - stop
            if risk > 0:
                rr = (tgt - last_price) / risk

        # ---------- mensagem tática ----------
        if long_ok:
            st.success(
                "✅ **Cenário favorável para LONG**\n\n"
                f"- **Entrada**: ~ **{_fmt(last_price)}**  \n"
                f"- **Stop**: ~ **{_fmt(stop)}**  \n"
                f"- **Alvo**: ~ **{_fmt(tgt)}**  \n"
                f"- **R:R**: {_fmt(rr)}\n\n"
                "Critérios atendidos: probabilidade ≥ limiar, médias alinhadas "
                "(SMA rápida > lenta), MACD positivo e RSI entre 45–70."
            )
        else:
            faltando = []
            ...
            msg = "⏳ **Sem entrada LONG agora.** Aguarde melhor alinhamento dos sinais."
            if faltando:
